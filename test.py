...
#We used a unique index per word as the inputs and targets of the network.
#To keep track of all this we used a helper class Lang which has word → index (word2index) and index → word (index2word) dictionaries,
#as well as a count of each word word2count to use to later replace rare words.
...
# Turn a Unicode string to plain ASCII, make everything lowercase, and trim most punctuation.
...
#To read the data file we split the file into lines, and then split lines into pairs.
def readLangs(lang1, lang2, reverse=False):
    print("Reading lines...")

    # Read the file and split into lines
    lines = open('data/Compositions2/train.txt', encoding='utf-8').\
        read().strip().split('\n')
...
# we trimmed the data set to only relatively short and simple sentences.
# Here the maximum length is 20 words and we’re filtering to sentences that translate to the form  "I am" or "He is" etc.
...
#AttnDecoderRNN focuses on a different part of the encoder’s outputs for every step of the decoder’s own outputs.
...
# trainIters calls train many times and occasionally print the progress (% of examples, time so far, estimated time) and average loss
...
# To evaluate, at each step, we simply feed the decoder’s predictions back to itself
...
output_words, attentions = evaluate(
    encoder1, attn_decoder1, sys.argv[1])
