...
#To keep track of unique index per word to use as the inputs and targets
# we will use a helper class called Lang which has word → index (word2index) and index → word (index2word) dictionaries,
...
#to simplify we will turn Unicode characters to ASCII,
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            ...
            loss += criterion(decoder_output, target_tensor[di])
            if decoder_input.item() == EOS_token: # detach from history as input
                break
...
# encoder1.save_state_dict('encoder1.pt')
# attn_decoder1.save_state_dict('attn_decoder1.pt')
